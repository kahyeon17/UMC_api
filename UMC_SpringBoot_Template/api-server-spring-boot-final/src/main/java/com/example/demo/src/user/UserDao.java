...
public class UserDao {
    ...
    // 1. User 테이블에 존재하는 전체 유저들의 정보 조회
    public List<GetUserRes> getUsers() {
        String getUsersQuery = "select * from User limit 3"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
        return this.jdbcTemplate.query(getUsersQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getInt("point"),
                        rs.getString("coupon"),
                        rs.getString("rank")) // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
        ); // 복수개의 회원정보들을 얻기 위해 jdbcTemplate 함수(Query, 객체 매핑 정보)의 결과 반환(동적쿼리가 아니므로 Parmas부분이 없음)
    }


    // 2. User 테이블에 존재하는 특정 유저의 정보 조회
    public GetUserRes getUser(int userIdx) {
        String getUserQuery = "select * from User where userIdx = ?"; // 해당 userIdx를 만족하는 유저를 조회x하는 쿼리문
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx,email,password,name from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (name, email, phoneNum, password) VALUES (?,?,?,?)";
        Object[] createUserParams = new Object[]{postUserReq.getName(), postUserReq.getEmail(), postUserReq.getPhoneNum(), postUserReq.getPassword()};
    ...
    // 5. 특정 유저 정보 수정 - phoneNum 변경 {userIdx}
    public int modifyUserCellN(PatchUserReq patchUserReq) {
        String modifyUserCellNQuery = "update User set phoneNum = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 phoneNum으로 변경한다.
        Object[] modifyUserCellNParams = new Object[]{patchUserReq.getPhoneNum(), patchUserReq.getUserIdx()};

        return this.jdbcTemplate.update(modifyUserCellNQuery, modifyUserCellNParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }


    // 6. 특정 유저의 찜 목록에 매장 추가 ?name=
    public int modifyUserDibs(PatchUserDibReq patchUserDibReq) {
        String modifyUserDibsQuery = "update User set dibStore = CONCAT(dibStore, ?) where name = ? "; // 해당 name을 만족하는 User의 찜한 Store에 해당 Store를 추가한다.
        Object[] modifyUserDibsParams = new Object[]{patchUserDibReq.getDibStore(), patchUserDibReq.getName()};

        return this.jdbcTemplate.update(modifyUserDibsQuery, modifyUserDibsParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }
